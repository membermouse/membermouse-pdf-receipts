...
<div class="mm-wrap" id="<?php echo $this->parent->_token.'_settings' ?>">
		...
		<div style="margin-left: 10px; width: 700px;">
			<div class="updated" style="padding: 10px; border-left-color: #690">
				<div style="margin-left: 20px;">
					<h2>
						<i class="fa fa-times" style="color: #c00"></i> <?= _mmpdft("PDF Receipts Not Active"); ?>
					</h2>
					
					<p style="font-size:14px;">
						<strong><?= _mmpdft("Complete the following in order to activate PDF receipts"); ?>:</strong>
					</p>
					...
					<?php } ?>
					<?= _mmpdft("Configure the PDF content below"); ?> or <a href="javascript:insertPDFConfigTemplate(true);"><?= _mmpdft("use the default configuration"); ?></a>.
					</p>
					...
					<i class="fa fa-square-o"></i> 
					<?php } ?>
					<?= _mmpdft("Configure the email template below"); ?> or <a href="javascript:insertEmailTemplate(true);"><?= _mmpdft("use the default template"); ?></a>.
					</p>
				</div>
			</div>
		</div>
    ...
    public function render_config_tab()
    {
		...
		<div style="margin-left: 10px; width: 700px;">
			...
			<div class="updated"
				...
				<form id="mm-pdf-config-form"  method='post'>
					<div style="margin-left: 20px;">
						<h2><?= _mmpdft("PDF Receipt Configuration"); ?></h2>
						<input name="mm_pdf_configuration" type="hidden" value="1" />
						
						<p><a href="javascript:insertPDFConfigTemplate(false);"><?= _mmpdft("Insert the Default Configuration"); ?></a></p>
						
						<p><strong><?= _mmpdft("Business Information"); ?></strong></p>
						...
						<p>
							<input id="mm-business-name" name="mm_business_name" type="text"
								...
								placeholder="<?= _mmpdft("Business Name"); ?>" />
						</p>
						<p>
						...
						<p>
							<input id="mm-business-tax-id" name="mm_business_tax_id" type="text"
								...
								placeholder="<?= _mmpdft("Business Tax ID"); ?>" /> (<em>optional</em>)
						</p>
						</div>
						
                		<div style="margin-top:20px;">
						<p><strong><?= _mmpdft("Receipt Footer"); ?></strong></p>
						<div style="margin-left:10px;">
						<p><?= _mmpdft("Area"); ?> 1</p>
						<p>
                		...
                		<div style="margin-top:20px;">
                		<strong><?= _mmpdft("Manually Entered Billing Details"); ?></strong></div>
                		<div style="margin-left:10px;">
                		<p><?= _mmpdft("You can allow the customer to manually enter their own billing details. To do this, select a custom field 
                		...
                		<p>
                			...
                			&nbsp;
                    		<a href="<?php echo $customFieldsURL ?>" target="_blank"><?= _mmpdft("configure custom fields"); ?></a>
						</p>
						<p>
                			<?php echo MM_Utils::getIcon('info-circle', 'blue', '1.3em', '2px'); ?> NOTE: <em><?= _mmpdft("only"); ?> 'Long Text' <?= _mmpdft("custom fields will appear in the dropdown above"); ?></em>
						</p>
						<?php } else { ?>
						<p>
							<?php echo MM_Utils::getIcon('info-circle', 'yellow', '1.3em', '2px'); ?>
							<?= _mmpdft("No custom fields with the type <em>Long Text</em> have been created."); ?> 
						...
						<div style="margin-top:20px;">
							<input type='submit' value='<?= _mmpdft("Save PDF Configuration"); ?>'
			...
			<div class="updated" style="padding: 10px; border-left-color: #f80">
				<form id="mm-email-template-form" method='post'>
					<div style="margin-left: 20px;">
						<h2><?= _mmpdft("Email Template Configuration"); ?></h2>
						<input name="mm_pdf_email_template" type="hidden" value="1" />
						<p><a href="javascript:insertEmailTemplate(false);"><?= _mmpdft("Insert the Default Email Template"); ?></a></p>
		...
		<a href="<?php echo $employeesUrl ?>" target="_blank"><?php echo _mmpdft("manage employees");?></a>
						...
						<?php echo _mmpdft("CC"); ?>
                		<div style="margin-left:10px;">
                		<p><?= _mmpdft("You can allow the customer to enter an email address to CC on PDF receipt emails. To do this, select a custom field 
						...
						<p>
                			<?php echo MM_Utils::getIcon('info-circle', 'blue', '1.3em', '2px'); ?> NOTE: <em><?= _mmpdft("only"); ?> 'Short Text' <?= _mmpdft("custom fields will appear in the dropdown above"); ?></em>
						</p>
						<?php } else { ?>
						<p>
							<?php echo MM_Utils::getIcon('info-circle', 'yellow', '1.3em', '2px'); ?>
							<?= _mmpdft("No custom fields with the type <em>Short Text</em> have been created."); ?>
							<a href="<?php echo $customFieldsURL ?>" target="_blank"><?php echo _mmpdft("Configure custom fields");?></a>.
						</p>
						<?php } ?>
						</p>
						<p>
							<input id="mm-email-subject" name="mm_email_subject" type="text"
								...
								placeholder="<?= _mmpdft("Email Subject"); ?>" />
						</p>
						<p>
		...
		<em><?php echo _mmpdft("Note: Only certain SmartTags can be used here"); ?></em><?php echo MM_Utils::getInfoIcon($validSmartTags); ?>
						...
						<p>
                			<?php echo MM_Utils::getIcon('paperclip', 'gray', '1.3em', '2px'); ?> <?= _mmpdft("PDF receipt will be attached to the email automatically"); ?>
                		</p>

						<input type='submit' value='<?= _mmpdft("Save Email Template"); ?>'
		...
		<div style="margin-left: 10px; width: 700px;">
			...
			<div class="updated" style="padding: 10px; border-left-color: #066cd2">
					...
					<div style="margin-left: 20px;">
						...
						<p>
							<strong><?= _mmpdft("Send test to the following email address"); ?>:</strong>
						</p>
						<p>
							<input name="mm_pdf_email_test_email" type="text"
								...
								placeholder="<?= _mmpdft("Enter email address"); ?>" />
						</p>
						<p>
		...
		<div style="margin-left: 10px; width: 700px;">
			...
			<div class="updated" style="padding: 10px; border-left-color: #066cd2">
					...
					<div style="margin-left: 20px; <?php echo (intval($crntStep) == 1) ? '':'display: none;'; ?>">
						...
						<p>
							<strong><?= _mmpdft("Enter order number to resend receipt for"); ?>:</strong>
						</p>
						<p>
							<input id="mm_pdf_resend_order_num" name="mm_pdf_resend_order_num" type="text"
								style="width: 100px; font-family: courier; font-size: 11px;" placeholder="<?= _mmpdft("Order"); ?> #" />
						</p>
						<p>
							<input type='submit' value='<?= _mmpdft("Preview"); ?>' class="mm-ui-button blue" />
						</p>
						</form>
					</div>
					...
					<div style="margin-left: 20px; <?php echo (intval($crntStep) == 2) ? '':'display: none;'; ?>">
						...
						?>
						<p>
							<strong><?= _mmpdft("Recipient Information"); ?></strong><br/>
							<?= _mmpdft("Name"); ?>: <?= $data['first_name']." ".$data['last_name']; ?><br/>
							<?= _mmpdft("To"); ?>: <?= $data['email']; ?><br/>
							<?= _mmpdft("CC"); ?>: <?php if(!empty($ccEmail)) { echo $ccEmail."<br/>"; } ?>
						</p>
						<p>
							<input name="mm_pdf_addl_cc_email" type="text" style="width: 251px; font-family: courier; 
							     font-size: 11px;" placeholder="<?= _mmpdft("Additional CC Email Address"); ?>" />
						</p>
						<p>
							<strong><?= _mmpdft("Order Information"); ?></strong><br/>
							<?= _mmpdft("Order"); ?> #: <?= $order->getOrderNumber(); ?><br/>
							<?= _mmpdft("Product"); ?>: <?= json_decode($data['order_products'], true)[0]['name'] ?><br/>
							<?= _mmpdft("Order Total"); ?>: <?= $order->getTotal(true); ?>
						</p>
						<p>
							<input type='submit' value='<?= _mmpdft("Confirm & Send Receipt"); ?>' class="mm-ui-button blue" />
						</p>
						<?php } else { ?>
						<p>
							<?= _mmpdft("No order specified. Click above to start over."); ?>
						</p>
						<?php } ?>
						</form>
					</div>
    ...
    public function render_info_tab()
    {
        ...
        <div style="margin-left: 20px;">
			<h2>
				<i class="fa fa-check" style="color: #690"></i> <?= _mmpdft("PDF Receipts Active"); ?>
			</h2>
			
			<?php $activityLogURL = MM_ModuleUtils::getUrl(MM_MODULE_LOGS, MM_MODULE_ACTIVITY_LOG); ?>
			
			<p><?= _mmpdft("An email with a PDF receipt attached will be sent to MemberMouse members when an initial or rebill payment occurs.
            ...
            function confirmReset() {
              if (confirm("<?= _mmpdft("Are you sure you want to reset the plugin to its initial state?"); ?>")) {
                jQuery("#mm_pdf_reset_plugin_form").submit();
              }
            }
            </script>
			<h3><?= _mmpdft("Reset Plugin to Initial State"); ?></h3>
			<p>
			<?= _mmpdft("Click the button below to reset the plugin to its initial state. This will delete the current configuration settings."); ?>
			</p>
			<form id="mm_pdf_reset_plugin_form" method='post' action='<?= remove_query_arg('tab'); ?>'>
				...
				<a onClick="confirmReset()" class="mm-ui-button red" style="padding:5px;"><?= _mmpdft("Reset Plugin"); ?></a>
			</form>
		</div>
		</div>
		</div>
